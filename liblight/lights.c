        ...
        struct light_state_t const* state)
{
    ...
    if (blink) {
        if (red) {
            if (write_int_on_off_time(LED_BLINK_FILE, red, onMS, offMS))
                write_int(LED_FILE, 0);
	}
        if (green) {
            if (write_int_on_off_time(LED_BLINK_FILE, green, onMS, offMS))
                write_int(LED_FILE, 0);
	}
    } else {
        write_int(LED_FILE, red);
        write_int(LED_FILE, green);
    }
